...
class PatchembedSuper(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = F.conv2d(x, self.sampled_weight.double(), self.sampled_bias.double(), stride=self.patch_size, padding=self.proj.padding, dilation=self.proj.dilation).flatten(2).transpose(1,2)
