...
import numpy as np
from nasbench_pytorch.model.model import Cell
...
# tss and sss
def extract_cell_features(model, inputs):
    cell_features = []
...
# 101
def extract_cell_features(network, inputs):
    cell_features = []  # Store feature maps for each layer

    # Pass input through initial stem convolution layers
    for i, layer in enumerate(network.layers):
        inputs = layer(inputs)
        
        # If this layer is a stem conv or cell, extract its feature map
        if isinstance(layer, (ConvBnRelu, Cell)):
            if inputs.requires_grad:  # Ensure gradients can be recorded
                inputs.retain_grad()
            cell_features.append(inputs)
...
def extract_cell_features(network, inputs):
    ...
    # Pass through stem layer
    s0 = s1 = network.stem(inputs)
    cell_features.append(s1)  # Save stem layer output feature map

    # Pass through each cell and save feature maps
    for i, cell in enumerate(network.cells):
        s0, s1 = s1, cell(s0, s1, network.drop_path_prob)
        cell_features.append(s1)  # Save cell layer output feature map

        # Save auxiliary feature map when auxiliary classifier exists and at the specified cell
        if i == 2 * network._layers // 3:
            if network._auxiliary and network.training:
                logits_aux = network.auxiliary_head(s1)
                cell_features.append(logits_aux)  # Save auxiliary classifier output (if exists)

    # Feature map before final global pooling layer
    pooled_output = network.global_pooling(s1)
    cell_features.append(pooled_output)
...
def compute_nas_score(model, gpu, trainloader, resolution, batch_size, init_method = 'kaiming_norm_fanin', fp16=False):
    ...
    layer_features = extract_cell_features(model,input_)
    ...
    # info['complexity'] = float(model.get_FLOPs(resolution)) # take info from api
    #AZ=info['expressivity']+info['progressivity']+info['trainability']
    return info 